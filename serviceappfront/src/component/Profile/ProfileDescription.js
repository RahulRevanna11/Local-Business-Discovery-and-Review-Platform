...
const ProfileDescription = (props) => {
  ...
  <div class="m-10 inline-block">
    <img src={props?.owner?.image} class="w-full md:w-[40vh] rounded-lg object-cover" alt="jkfd" />
  </div>
  <div class="mt-3 lg:ml-[20vh] flex flex-col gap max-w-full md:max-w-[60vw] inline">
    <h2 class="text-black text-3xl font-bold from-stone-300 p-2">{props.name}</h2>
    <div class="flex flex-row gap-3 justify-center">
      <p class="bg bg-green-500 inline px-2 py-1 rounded-lg">{props?.avgRating}</p>
      <ReactStars
        count={5}
        size={30}
        isHalf={true}
        ...
        activeColor="#ffd700"
      />
      <p class="text-lg text-gray-600 font-bold mt-2">{(props?.contact)?props?.contact:0}</p>
    </div>
    <div class="flex mt-2 gap-4">
      <div class="font-semibold flex gap-2">
        <MdOutlineInsights />
        {props.totalEnquires ? props.totalEnquires : ""} People recently enquired
      </div>
      <p class="bg-gray-500 inline p-1 rounded-md">Since <span className="text-white px-1">{props.year_of_establishment ? props.year_of_establishment : 0.1}</span> Year</p>
    </div>
    <div class="flex flex-col md:flex-row justify-between mt-4 font-semibol gap-4 md:mt-1">
      <div class="bg-green-600 rounded-xl p-2 flex justify-center hover:bg-green-700 hover:scale-110 transition duration-150 ease-out hover:ease-in">
        ...
        <p class="text-white font-bold text-xl md:text-md">{(props?.contact)?props?.contact:'not available'}</p>
      </div>
      ...
      <div class="text-md p-2 border-gray-100 border-2 rounded-lg hover:scale-110 transition duration-150 ease-out hover:ease-in flex gap-2">
        <RiAiGenerate class="text-xl mt-1 text-red-500" />
        <p onClick={showToast}>Tap to Rate</p>
      </div>
    ...
    <div class="text-2xl bg-blue-400 rounded-xl my-5 text-white font-bold inline w-full md:w-44">
      <p class="p-2 inline">Enquire Now</p>
    </div>
  </div>
</div>
